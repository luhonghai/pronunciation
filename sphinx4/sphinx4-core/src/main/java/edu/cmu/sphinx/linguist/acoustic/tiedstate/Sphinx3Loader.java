...
public class Sphinx3Loader implements Loader {
    ...
    private static final int MAX_INSTANCE_CACHE_SIZE = 3;
    /**
     * Cache instance
     */
    private static final Map<String, Sphinx3Loader> instances = new ConcurrentHashMap<String, Sphinx3Loader>();

    private static Sphinx3Loader getInstance(final String location, final Sphinx3Loader defaultObject) {
        if (instances.containsKey(location))
            return instances.get(location);
        return defaultObject;
    }

    private static void setInstance(final String location, final Sphinx3Loader loader) {
        if (instances.size() >= MAX_INSTANCE_CACHE_SIZE) {
            String firstKey = instances.keySet().iterator().next();
            instances.remove(firstKey);
        }
        instances.put(location, loader);
    }
    ...
    public void load() throws IOException {
        if (getInstance(getLocation(), null) == null) {
            logger.info("No Sphinx3Loader found for location " + getLocation() + ". Try to load.");
            ...
            contextIndependentUnits = new LinkedHashMap<String, Unit>();
                ...
                setInstance(getLocation(), this);

        } else {
            logger.info("Found Sphinx3Loader cache for location " + getLocation() + ". Skip by default.");
        }
    }
    ...
    public Map<String, Unit> getContextIndependentUnits() {
        return getInstance(getLocation(), this).contextIndependentUnits;
    }
    ...
    public Pool<float[]> getMeansPool() {
        return getInstance(getLocation(), this).meansPool;
    }

    public Pool<float[][]> getMeansTransformationMatrixPool() {
        return getInstance(getLocation(), this).meanTransformationMatrixPool;
    }

    public Pool<float[]> getMeansTransformationVectorPool() {
        return getInstance(getLocation(), this).meanTransformationVectorPool;
    }

    public Pool<float[]> getVariancePool() {
        return getInstance(getLocation(), this).variancePool;
    }

    public Pool<float[][]> getVarianceTransformationMatrixPool() {
        return getInstance(getLocation(), this).varianceTransformationMatrixPool;
    }

    public Pool<float[]> getVarianceTransformationVectorPool() {
        return getInstance(getLocation(), this).varianceTransformationVectorPool;
    }

    public Pool<float[]> getMixtureWeightPool() {
        return getInstance(getLocation(), this).mixtureWeightsPool;
    }

    public Pool<float[][]> getTransitionMatrixPool() {
        return getInstance(getLocation(), this).transitionsPool;
    }

    public float[][] getTransformMatrix() {
        return getInstance(getLocation(), this).transformMatrix;
    }

    public Pool<Senone> getSenonePool() {
        return getInstance(getLocation(), this).senonePool;
    }
    ...
    public HMMManager getHMMManager() {
        return getInstance(getLocation(), this).hmmManager;
    }

    public void logInfo() {
        logger.info("Loading tied-state acoustic model from: " + location);
        getInstance(getLocation(), this).meansPool.logInfo(logger);
        getInstance(getLocation(), this).variancePool.logInfo(logger);
        getInstance(getLocation(), this).transitionsPool.logInfo(logger);
        getInstance(getLocation(), this).senonePool.logInfo(logger);

        if (getInstance(getLocation(), this).meanTransformationMatrixPool != null)
            getInstance(getLocation(), this).meanTransformationMatrixPool.logInfo(logger);
        if (getInstance(getLocation(), this).meanTransformationVectorPool != null)
            getInstance(getLocation(), this).meanTransformationVectorPool.logInfo(logger);
        if (getInstance(getLocation(), this).varianceTransformationMatrixPool != null)
            getInstance(getLocation(), this).varianceTransformationMatrixPool.logInfo(logger);
        if (getInstance(getLocation(), this).varianceTransformationVectorPool != null)
            getInstance(getLocation(), this).varianceTransformationVectorPool.logInfo(logger);

        getInstance(getLocation(), this).mixtureWeightsPool.logInfo(logger);
        getInstance(getLocation(), this).senonePool.logInfo(logger);
        logger.info("Context Independent Unit Entries: "
                + getInstance(getLocation(), this).contextIndependentUnits.size());
        getInstance(getLocation(), this).hmmManager.logInfo(logger);
    }

    public Properties getProperties() {
        return getInstance(getLocation(), this).modelProps;
    }
    ...
    public void update(Transform transform, ClusteredDensityFileData clusters) {
        for (int index = 0; index < getInstance(getLocation(), this).meansPool.size(); index++) {
            int transformClass = clusters.getClassIndex(index);
            float[] tmean = new float[getVectorLength()[0]];
            float[] mean = getInstance(getLocation(), this).meansPool.get(index);
