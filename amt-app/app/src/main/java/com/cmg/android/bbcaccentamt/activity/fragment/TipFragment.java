...
import com.cmg.android.bbcaccentamt.data.TipsContainer;
import com.cmg.android.bbcaccentamt.data.UserVoiceModel;
import com.cmg.android.bbcaccentamt.utils.RandomHelper;
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.btnRecordTip:
                Intent intent = new Intent(HistoryFragment.ON_HISTORY_LIST_CLICK);
                Gson gson = new Gson();
                UserVoiceModel model = new UserVoiceModel();
                model.setWord(currentTip.getWords().get(currentTipIndex));
                intent.putExtra(BaseActivity.USER_VOICE_MODEL, gson.toJson(model));
                intent.putExtra(ACTION_TYPE, HistoryFragment.CLICK_RECORD_BUTTON);
