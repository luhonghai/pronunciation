...
public class CMTDAO extends DataAccess<CourseMappingTeacher> {
   ...
   /* *//**
     ...
     * @throws Exception
     */
    public ArrayList<CourseDTO> searchHeaderMyCourse(String cpId, String tId, String cName) throws Exception{
        PersistenceManager pm = PersistenceManagerHelper.get();
        SQL sqlUtil = new SQL();
        String sql = sqlUtil.getSqlDisplayCourseSearchHeaderInMyCourse(tId, cpId, cName);
        System.out.println("sql search header in my course page : " + sql);
        ArrayList<CourseDTO> list = new ArrayList<CourseDTO>();
        Query q = pm.newQuery("javax.jdo.query.SQL", sql);
        try {
            List<Object> tmp = (List<Object>) q.execute();
     ...
     * @throws Exception
     */
    public ArrayList<CourseDTO> searchDetailMyCourse(String cpId, String tId,
                                                     String cName, String companyName,
                                                     String dateFrom, String dateTo){
        PersistenceManager pm = PersistenceManagerHelper.get();
        SQL sqlUtil = new SQL();
        String sql = sqlUtil.getSqlDisplayCourseSearchDetailInMyCourse(tId,cpId,companyName,cName,dateFrom,dateTo);
        System.out.println("sql search detail in my course page : " + sql);
        ArrayList<CourseDTO> list = new ArrayList<CourseDTO>();
        Query q = pm.newQuery("javax.jdo.query.SQL", sql);
        try {
            List<Object> tmp = (List<Object>) q.execute();
    ...
    public ArrayList<CourseDTO> searchCourseHeader(String status,String name){
        PersistenceManager pm = PersistenceManagerHelper.get();
        SQL sqlUtil = new SQL();
        String sql = sqlUtil.getSqlSearchCourseHeader(status, name);
        System.out.println("sql search course header : " +  sql);
        ArrayList<CourseDTO> list = new ArrayList<CourseDTO>();
        Query q = pm.newQuery("javax.jdo.query.SQL", sql);
        try {
            List<Object> tmp = (List<Object>) q.execute();
            if(tmp!=null && tmp.size() > 0){
                for(Object obj : tmp){
                    ...
                    if(array[6]!=null){
                        dto.setSr(array[6].toString());
                    }
                    if(array[7]!=null){
                        dto.setIdCompany(array[7].toString());
                    }
                    if(array[8]!=null){
                        dto.setIdTeacher(array[8].toString());
                    }
                    list.add(dto);
                }
            }
