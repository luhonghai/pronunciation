...
public class RecordingView extends View {
    ...
    private void drawPingCycle() {
        drawPingCycle(MAX_PING_CYCLE_TIME, 100.0f, false, true, true);
    }

    private synchronized void drawPingCycle(long maxPingCycleTime, float scoreRadio,
                                            boolean showScore, boolean doInvalidate, boolean enableAnimation) {
        if (!initBitmap()) return;
        if (pingStartTime == -1) {
    ...
    private class AnimationRunable implements Runnable {
        ...
        private AnimationRunable(Activity activity, long maxPingCycleTime, float scoreRadio, boolean showScore) {
        ...
        public void run() {
            activity.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    drawPingCycle(maxPingCycleTime, scoreRadio, showScore, true, true);
    ...
    public void startPingAnimation(Activity activity) {
        startPingAnimation(activity, MAX_PING_CYCLE_TIME, 100.0f, false);
    }

    public void startPingAnimation(Activity activity, long maxPingCycleTime, float scoreRadio, boolean showScore) {
        stopPingAnimation();
        stopPingCycle();
        if (showScore) {
        ...
        isMaxScore = false;
        animationRunable = new AnimationRunable(activity, maxPingCycleTime, scoreRadio, showScore);
