<?php
	...
	if($errflag == false ){
		...
		if($result){
			...
			if(mysql_num_rows($result)==1) {
				...
				// user exists checking whether it is a student or not
				...
				if(strcmp($type[0], '1')==0) {
					// already an existing student member
					if($debug) printf('## user as a student already registered! ##');
				
					$errmsg_arr[] = 'The email: <strong>'.$useremail.'</strong> is already registered as a student!';
					$errflag = true;
					$errtype = 'user_exists';
				} else {
					// existing non-student member, adding extra type, but ignoring the password
					$type[0]='1'; // add student type to the current user
					...
					if($update_result) {
						...
						$errmsg_arr[] = 'Update to student user failed! Please retry later!';
						$errflag = true;
					}
				}
				...
				// insert the new user
				$insert_qry = "insert into users (email, password, type, time) values ('".$useremail."', '".$password."', '10000000', NOW())";
	...
	if($errflag == false) {
		// as registered as a student, thus go to student page directly
