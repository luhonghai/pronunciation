...
if len(sys.argv) != 5:
        print '\nUsage: python test_phrase_random_feedback.py <input_phonesegdir> <transcription.insent> <TIMIT_stats.txt> <CMU_phonelist>'
        print 'Note: Check folder "files" for required arguments\n'
	sys.exit(1)
				...
				while True:
					if(allphones[indx][0]==s[3] and int(allphones[indx][1])==pos):
						...
						indx = indx+1;
                                ...
                                z_score = abs((phone_duration - float(allphones[indx][4]))/float(allphones[indx][5]))
                                st_score1 = 5 - t_score
                                st_score2 = 5 - z_score
				if(st_score2<0):
					st_score2 = 0
