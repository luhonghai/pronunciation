...
if len(sys.argv) != 4:
        print '\n Usage: python calc_statistics.py <input_phonesegdir> <complete_phone_list> <output_data_file> \n'
        sys.exit(1)
...
str_prnt = 'phone \t Position \t Mean Score \t Std. Score \t Mean duration \t Std. duration \t count\n'
out_phone.write(str_prnt);
count=0;
i=0
while 1:
	if i==len(cmuphones):
		...
		for cnt in range(0,3):
			...
			str_prnt = str(cmuphones[i][0])+"\t"+str(cnt)+"\t"+str(cmuphones[i][1])+"\t"+str(cmuphones[i][2])+"\t"+str(cmuphones[i][3])+"\t"+str(cmuphones[i][4])+"\t"+str(cmuphones[i][5])
			out_phone.write(str_prnt+'\n')
		i=i+1		
